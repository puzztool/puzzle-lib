...
class Character {
  ...
  toString () {
    return this._ensureLookup().exactString;
  }
  ...
  _ensureLookup () {
    if (!this._lookup) {
      ...
      this._lookup.exactString = this._lookup.exact.map((value) => value.toString()).join('/');
    }
  ...
  _update () {
    while (this._processPosition < this._chars.length) {
      ...
      switch (ch) {
        ...
        default:
          this._state.previousWhitespace = false;
          const category = this._state.number ? Category.Number : Category.Letter;
          const exact = Data.instance().lookup(ch, category).exact[0];
          this._currentStr += exact.toString();
      }
